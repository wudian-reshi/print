...
function printCountData (element, success, error) {
	...
	LODOP.SET_PRINTER_INDEX(getPrint('Canon iP2700 series'))
...
function printTags(data, success, error) {
	let print = getPrint('Xprinter XP-365B');
	console.log('打印机下标：', print);
    // 可以直接使用 LODOP
    for(var i =0;i<data.length;i++){
		...
		`
		var initName = data[i].date + '-' + data[i].deviceCode + '-' + data[i].latticeNum;
        LODOP.PRINT_INIT(initName);
		LODOP.SET_PRINTER_INDEX(print)
		
		
		LODOP.SET_PRINT_PAGESIZE(1, '70mm', '50mm')
		LODOP.SET_PRINT_STYLE("FontName", '微软雅黑 Bold')
		LODOP.ADD_PRINT_HTM(30,5,"100%","80%",html); //左右边距都是10%    
        //LODOP.PREVIEW();
        LODOP.PRINT();
    }
    return success();
...
function getPrint(printName) {
	let printList = LODOP.GET_PRINTER_COUNT();
	console.log('打印机列表:', printList);
	for (let i = 0; i < printList; i++) {
		console.log(LODOP.GET_PRINTER_NAME(i));
		if (LODOP.GET_PRINTER_NAME(i) === printName) {
			return i;
		}
	}
}